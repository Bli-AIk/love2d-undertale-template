...
function tween.CreateTween(variableSetter, type, direction, begin, final, duration, delay, destroy)
    local animation = {}
    ...
    animation.delay = (delay or 0)
    animation.destroy = (destroy or duration)
...
function tween.Update(dt)
    for i = #tween.animations, 1, -1 do
        ...
        if (animation.time <= animation.duration) then
            ...
            if (animation.time >= animation.destroy + animation.delay) then
                animation.time = animation.duration + animation.delay + 1
            end
            ...
            if (name and type(name) == "string") then
                ...
                if not ok then
                    print("Error in variableSetter: " .. tostring(err))
                end
            else

...
local functions = {
    ...
    BubbleSize = function(typer, w, h)
        ...
        typer.bubble.main_rect_v:MoveTo(
            typer.bubble.main_rect_h.x + typer.bubble.main_rect_h.xscale / 2,
            typer.bubble.main_rect_h.y + typer.bubble.main_rect_h.yscale / 2
        )
        typer.bubble.main_rect_v:Scale(w - 40, h)
        typer.bubble.corner_ur:MoveTo(
            typer.bubble.main_rect_h.x + typer.bubble.main_rect_h.xscale - 10,
            typer.bubble.main_rect_h.y - 10
        )
        typer.bubble.corner_dl:MoveTo(
            typer.bubble.main_rect_h.x + 10,
            typer.bubble.main_rect_h.y + typer.bubble.main_rect_h.yscale + 10
        )
        typer.bubble.corner_dr:MoveTo(
            typer.bubble.main_rect_h.x + typer.bubble.main_rect_h.xscale - 10,
            typer.bubble.main_rect_h.y + typer.bubble.main_rect_h.yscale + 10
        )
    end,
    ...
    Destroy = function(typer)
        ...
        do
            if (typers.instances[i] == typer) then
                ...
                for j = #typer.letters, 1, -1 do
                    local letter = typer.letters[j]
                    if letter and letter.char and type(letter.char.release) == "function" then
                        letter.char:release()
                    end
                    table.remove(typer.letters, j)
                end
                ...
                typer.sleep = 0
                typer.interval = 0
                if (typer.realfont) then
                    typer.realfont:release()
                    typer.realfont = nil
                end
...
local functions_instant = {
    ...
    MoveTo = function (typer, x, y)
        ...
        typer.y = y or typer.y
        if typer.parsed then
            typer:Reparse()
        end
    end,
    Destroy = function(typer)
        for k = #layers.objects, 1, -1
        do
            if (layers.objects[k] == typer) then
                table.remove(layers.objects, k)
            end
        end
        for i = #typers.instances, 1, -1
        do
            if (typers.instances[i] == typer) then
                ...
                if typer.fontCache then
                    for key, font in pairs(typer.fontCache) do
                        font:release()
                        font = nil
                    end
                    typer.fontCache = {} -- 清空缓存表
                end

                for v = #typer.parsed, 1, -1
                do
                    local piece = typer.parsed[v]
                    piece = {}
                end
                typer.parsed = {}
                table.remove(typers.instances, i)
            ...
            if (typer.time >= typer.interval and typer.counter <= sentence:len()) then
                ...
                do
                    ...
                    do
                        ...
                        typer.font = getFont(typer.fonts[typer.currentfont], typer.fontsize[typer.currentfont])
                        local width = typer.realfont:getWidth(" ")
                    ...
                    while (letter == "\n") do
                        ...
                        typer.font = getFont(typer.fonts[typer.currentfont], typer.fontsize[typer.currentfont])
                        local height = typer.realfont:getHeight()
                    ...
                    do
                        ...
                        if (command_complex) then
                            ...
                            elseif (command_head == "bubblesize") then
                                local w, h
                                w = command_body:sub(1, command_body:find(",") - 1)
                                h = command_body:sub(command_body:find(",") + 1, -1)
                                typer.bubble.main_rect_h:Scale(tonumber(w), tonumber(h) - 40)
                                typer.bubble.main_rect_v:MoveTo(
                                    typer.bubble.main_rect_h.x + typer.bubble.main_rect_h.xscale / 2,
                                    typer.bubble.main_rect_h.y + typer.bubble.main_rect_h.yscale / 2
                                )
                                typer.bubble.main_rect_v:Scale(tonumber(w) - 40, tonumber(h))
                                typer.bubble.corner_ur:MoveTo(
                                    typer.bubble.main_rect_h.x + typer.bubble.main_rect_h.xscale - 10,
                                    typer.bubble.main_rect_h.y - 10
                                )
                    ...
                    if (typer.cantype) then
                        ...
                        typer.realfont = getFont(typer.fonts[typer.currentfont], typer.fontsize[typer.currentfont])
                        typer.realfont:setFilter("nearest", "nearest")
                        ...
                        local width = typer.realfont:getWidth(letter)
                        local height = typer.realfont:getHeight(letter)
                        ...
                        instance.font = typer.realfont
                        instance.char = love.graphics.newText(typer.realfont, letter)
...
function typers.DrawText(text, position, layer)
    ...
    function typer:Draw()
        ...
        do
            ...
            typer.realfont = getFont(piece.font, piece.size)
            love.graphics.setFont(typer.realfont)
            local charWidth = typer.realfont:getWidth(piece.char)
...
function typers.clear()
    for i = #typers.instances, 1, -1 do
        local typer = typers.instances[i]
        for _, font in pairs(typer.fontCache) do
            font:release()
        end
        typer:Destroy() -- 确保实例被销毁
    end
    typers.instances = {} -- 清空实例列表
end

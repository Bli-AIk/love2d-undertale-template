...
battle = require("Scripts.Libraries.Game.Encounter")
...
local enctemp = (battle.STATE == "ACTIONSELECT") and battle.EncounterText or ""
local encounterTyper = typers.CreateText(enctemp, {60, 270}, 13, {400, 150}, "none")
...
local bar_krs = {}
local _kr_configuration = false
local _kr_image = typers.DrawText("KR", {0, 0}, 0)
...
function SCENE.update(dt)
    ...
    if (particle_time % 5 == 0) then
        local square = sprites.CreateSprite("px.png", -90)
        square.alpha = 1

        local scrand = math.random(5, 15)
        square:Scale(scrand, scrand)
        square:MoveTo(math.random(0, 640), -10)
        square.velocity.y = math.random(1, 3)
        square.rotation = math.random(0, 359)

        table.insert(particles, square)
    end

...
battle = require("Scripts.Libraries.Game.Spider")
...
battle.wave = nil
battle.nextwave = "Spider/wave_intro"
global:SetVariable("PlayerPosition", {0, 0})
_CAMERA_:setPosition(0, 0)
local blacktop = sprites.CreateSprite("px.png", 20000000)
...
local function HandleActions(enemy_name, action)
    if (enemy_name == "[preset=chinese]蜘蛛") then
        if (action == "[preset=chinese][offsetX=4]查看") then
            BattleDialogue({
                "* [fontIndex:2][pattern:chinese]一只大螃蟹而已。"
            })
        end
    end
end
...
function SCENE.update(dt)
    ...
    if (SPIDERATTACKED) then
        SPIDERGTIME = SPIDERGTIME + 1
        if (SPIDERGTIME == 1) then
            SPIDER._g = 3
            SPIDER.velocity.x = math.random(-2, 2)
            SPIDER._rrs = math.random(-3, 3)
        elseif (SPIDERGTIME > 1) then
            SPIDER._g = SPIDER._g - 0.15
            SPIDER:Move(0, -SPIDER._g)
            SPIDER.rotation = SPIDER.rotation + SPIDER._rrs
        end
    end
...
function SCENE.clear()
    package.loaded["Scripts.Libraries.Game.Spider"] = nil
